...
def build_mnist_cnn():
    ...
    ret.add(layers.Conv2D(32, input_shape=(28, 28, 1),
                          kernel_size=3, activation='relu', name="conv1"))
...
def build_min_conv_layer(filters=1, kernel_size=1):
    ...
    ret.add(layers.Conv2D(filters, input_shape=(28, 28, 1),
                          kernel_size=kernel_size, name="conv1"))
...
def build_single_conv_layer():
    ...
    ret.add(layers.Conv2D(32, input_shape=(28, 28, 1),
                          kernel_size=3, activation='relu', name="conv"))
    ret.add(layers.Dense(10, activation='softmax', name="softmax"))
...
def build_cifar10(shape=(32, 32, 3), classes=10, name='cifar10'):
    return get_resnet_v1(shape, 32, classes, name)


def get_pretrained_imagenet(name="imagenet-pretrained"):
    ...
    ret = keras.Model(inputs=m.inputs, outputs=m.outputs,
                      name=name)
    return ret


def get_empty_imagenet(name="imagenet-empty"):
    m = keras.applications.ResNet50V2(
        include_top=True,
        weights=None,
        input_tensor=None,
